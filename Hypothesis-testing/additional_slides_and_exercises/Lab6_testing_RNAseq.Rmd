...
In this lab, we will become more familiar with count data in RNA-Seq high-throughput
...
on the cells' transcriptome. We already quickly explored this dataset 
...
In this section, we get a glimpse of single cell RNA-Seq data and check that the Negative Binomial model used for bulk RNA-Seq can often work well also for single cell data.

Our starting point is a dataset from 10X Genomics of peripheral blood mononuclear cells (PBMC).
...
Note that the `sce` object is of class `SingleCellExperiment`, which is the Bioconductor class for working with single cell data.  It contains measurements for 32738 genes across 2900 cells. The scale of the datasets is a key difference compared to bulk RNA-Seq, were typically we only have a handful of samples that have been measured.

As we did for the Pasilla bulk RNA-Seq dataset previously, we now seek to explore the mean-variance relationship. To this end, we calculate the variance and mean of each gene. Note that a more refined analysis would first normalize the counts across cells.

```{r}
sce_counts <- as.matrix(counts(sce))
sce_vars <- rowVars(sce_counts)
sce_means <- rowMeans(sce_counts)
sce_df <- data.frame(mean = sce_means, var=sce_vars)
```

We filter out genes that had either no counts at all (i.e., were never measured) and some with a high count (notice that the actual counts in single cell RNA-Seq are typically a lot smaller than in bulk RNA-Seq):

```{r}
filt_idx <-  (sce_means == 0) | (sce_means > 20)
sce_filt <- sce[!filt_idx,]
sce_filt_df <- sce_df[!filt_idx,]
sce_count_filt <- sce_counts[!filt_idx, ]
```

Let us plot the mean-variance relationship:
...
We see that most of the genes indeed follows the relationship we would expect from Negative Binomial data, i.e.,

$$V(\mu) = \mu + \alpha \mu^2$$

For single cell RNA-Seq, we have enough data, that we could fit the two parameters $\alpha, \mu$ of the Negative Binomial model individually for each gene. For example we could use the `fitdistr` function of the `MASS` package and apply it to each row of the count matrix. This is to be contrasted with bulk RNA-Seq, where Empirical Bayes methods are necessary to stabilize estimates for individual genes by sharing information across genes.

Instead, here, we do a simpler analysis (similar to an analysis by [Svensson](https://www.biorxiv.org/content/biorxiv/early/2019/03/19/582064.full.pdf). First, for each gene $i$, we estimate the parameter $\mu_i$ of the Negative Binomial just by the mean of the counts for that gene. Second, we use the same $\alpha$ across all genes (for the sake of illustration; this should typically not be done for a real analysis!). Then, we can directly estimate $\alpha$ by fitting the curve $V(\mu) = \mu + \alpha \mu^2$. We do this using the `rlm` function in the `MASS` package. This is a robust version of `lm` that is not as influenced by outlying points. We do this because in the mean-variance plot we saw some genes that did not seem to follow the curve of most genes.


```{r}
rlm_fit <- rlm(var-mean ~ 0  + I(mean^2), data=sce_filt_df, maxit=100)
```
We use the syntax above to force the model to only fit the parameter `alpha`, rather than also fitting an intercept. The `I()` protects the input, that is we inform the model that we want a coefficient for `mean^2`. Without `I()`, operators such as `*, +, ^` have a different meaning within a formula and so the output would be different.

Let us extract the estimated $\alpha$:
```{r}
alpha_dispersion <- coef(rlm_fit)
alpha_dispersion
```

We now compute the variance predicted from the Negative Binomial model, and overlay it on our original plot of the mean-variance relationship:

```{r fig.width = 4, fig.height = 4}
sce_filt_df <- dplyr::mutate(sce_filt_df, fitted_var = mean + alpha_dispersion*mean^2)
ggplot(sce_filt_df, aes(x=mean,y=var)) + 
                 geom_point(alpha=0.1) + 
                 geom_line(aes(y=fitted_var), col="red")
```

We see a decent fit for most genes. Next we explore another aspect of single cell RNA-Seq data and see if the Negative Binomial model can also explain it: The large number of zeros. For each gene, let us compute the proportion of cells for which the gene was not measured at all (i.e., the count is 0).



```{r}
zero_prop <- rowMeans( sce_count_filt == 0 )
```

```{r fig.width = 4, fig.height = 4}
ggplot(data.frame(zero_prop = zero_prop), aes(x=zero_prop)) +
                                    geom_histogram(binwidth = 0.05, fill = "Royalblue", boundary = 0) + 
                                    xlab("Proportion of zeros")
```

Is such a large number of zeros consistent with the Negative Binomial model? We can check this by comparing the observed proportion of zeros to the one predicted by the Negative Binomial model! We can get the latter prediction by the `dnbinom` function. We need to be slightly careful, since `dnbinom` uses a different parametrization of the Negative Binomial distribution (read the documentation!). 

```{r}
sce_filt_df <- dplyr::mutate(sce_filt_df, zero_prop = zero_prop,
                      theoretical_zero_prop = dnbinom(0, mu=mean, size=1/alpha_dispersion))
```


```{r fig.width = 4, fig.height = 4}
ggplot(sce_filt_df, aes(x=mean, y=zero_prop))  +
                         geom_point(alpha=0.1) +
...
As for the mean-variance relationship, we see that for most genes indeed the Negative Binomial provides a great fit. The deviating genes might correspond to genes that show heterogeneous biological activity across PBMCs. This is why [Svensson (2019)](https://www.biorxiv.org/content/biorxiv/early/2019/03/19/582064.full.pdf) conducts an analysis analogous to the one above on negative control data, i.e., datasets where no biological heterogeneity is expected. His goal is to demonstrate that the Negative Binomial distribution often provides an almost perfect fit for (UMI) single cell data, for both the number of zeros and the mean-variance relationship.
